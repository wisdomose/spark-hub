...
export default function PotterDashboard({
  active,
  children,
}: {
  active: string;
  ...
  const { loading, loggedIn, logout, user } = usePotter();
  ...
  useEffect(() => {
    if (loading) return;
    if ((!loading && !loggedIn) || user?.role !== ROLES.POTTER) {
      logout();
      router.replace("/");
    }
  }, [loading, loggedIn]);
